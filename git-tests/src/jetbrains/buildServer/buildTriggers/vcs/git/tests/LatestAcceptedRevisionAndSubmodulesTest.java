...
import static jetbrains.buildServer.buildTriggers.vcs.git.tests.LatestAcceptedRevisionTest.ensureFetchPerformed;
...
public class LatestAcceptedRevisionAndSubmodulesTest extends BaseRemoteRepositoryTest {
  ...
  public void test_rule_submodule_added() throws VcsException {
    ...
    ensureFetchPerformed(support, root, "refs/heads/master", "0cc4726cc3f1a3db11f8c8a47acb69d3515d402a");

    String rev = support.getCollectChangesPolicy().getLatestRevisionAcceptedByCheckoutRules(root, new CheckoutRules("+:module2"),
                                                                                            "0cc4726cc3f1a3db11f8c8a47acb69d3515d402a",
                                                                                            Collections.emptySet());
    then(rev).isEqualTo("4ecc4f42602de72476ebe0bc3bc58fac65960179");
    ...
    rev = support.getCollectChangesPolicy().getLatestRevisionAcceptedByCheckoutRules(root, new CheckoutRules("+:module1"),
                                                                                     ...
                                                                                     Collections.emptySet());
    then(rev).isEqualTo("d7ea7f829cd1c230d740de548817543737c157d4");
  }

  public void test_rule_submodule_changed() throws VcsException {
    GitVcsSupport support = git();
    VcsRoot root = vcsRoot().withFetchUrl(GitUtils.toURL(myRepo)).withSubmodulePolicy(SubmodulesCheckoutPolicy.CHECKOUT).build();
    ensureFetchPerformed(support, root, "refs/heads/master", "dc65ba0f3053d2248013e6dd0be3a61d57de44af");

    String rev = support.getCollectChangesPolicy().getLatestRevisionAcceptedByCheckoutRules(root, new CheckoutRules("+:module2"),
                                                                                            "02a41094cf4993c43230c8738acdf29e91cbb0d6",
                                                                                            Collections.emptySet());
    then(rev).isEqualTo("57f0352c60269c14f02f2863c7a35cf8573aead0");
    ...
    rev = support.getCollectChangesPolicy().getLatestRevisionAcceptedByCheckoutRules(root, new CheckoutRules("+:module2\n-:module2/test"),
                                                                                     ...
                                                                                     Collections.emptySet());
    then(rev).isEqualTo("af8389af87d5af3802bd71e9d4a7df963c68ace8");
  }
