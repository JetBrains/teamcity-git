...
public class LoggingGitMetaFactory implements GitMetaFactory {
  ...
  @NotNull
  public GitFactory createFactory(@NotNull GitAgentSSHService sshService, @NotNull AgentPluginConfig config) {
    myInvokedMethods.clear();//reset methods on every updateSources call
    return new GitFactoryProxy(sshService, config, myInvokedMethods);
  }
